...
from geolocation.raycaster import EARTH_RADIUS_M
from geolocation.transforms import rotation_matrix_zyx
from models import CameraIntrinsics, Detection, Frame, GeoPoint, Telemetry
...
class AttitudeAwareGroundPlaneRaycaster:
    ...
    def geolocate(self, detection: Detection, frame: Frame, telemetry: Telemetry) -> GeoPoint:
        ...
        scale = telemetry.alt_agl_m / down
        ...
        alt_msl = telemetry.alt_msl_m - telemetry.alt_agl_m

        attitude_penalty = 0.02 * telemetry.alt_agl_m * (
            abs(math.sin(math.radians(telemetry.roll_deg))) + abs(math.sin(math.radians(telemetry.pitch_deg)))
        )
        cep_m = max(3.0, 0.03 * telemetry.alt_agl_m + attitude_penalty)
